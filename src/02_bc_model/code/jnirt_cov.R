...
#' 
#' @export jnirt
jnirt_cov<- function(X, Y,family_network,family_responses, K=2,D=1, model = "bilinear",
  ...
  { 
    ...
    {  
      ...
      if(model=="UV"){
        ...
        ALPHATHETA<-svd(Theta %*% t(Alpha))
        Theta_1<-ALPHATHETA$u[,seq(1,D,length=D)]%*%diag(sqrt(ALPHATHETA$d[seq(1,D,length=D)]),nrow=D)
        
        S<-cov(cbind(U_1,V_1, Theta_1))
        Su = matrix(S[1:(K*2),1:(K*2)], nrow=K*2, ncol=K*2)
        Stheta = matrix(S[(K*2+1):(K*2+D),(K*2+1):(K*2+D) ], nrow=D, ncol=D)
        Sutheta =matrix(S[(K*2+1):(K*2+D),1:(K*2) ], nrow = D, ncol = K*2)
      }
      ...
      if(model=="bilinear"){
        ...
        S<-cov(cbind(U_1, Theta_1))
        Su = matrix(S[1:K,1:K], nrow=K, ncol=K)
        Stheta = matrix(S[(K+1):(K+D),(K+1):(K+D) ], nrow=D, ncol=D)
        Sutheta =matrix(S[(K+1):(K+D),1:K ], nrow = D, ncol = K)
