...
#' 
#' @export jnirt
jnirt_1<- function(X, Y,family_network,family_responses, K=2,D=1, model = "bilinear",
...
{ 
  ...
  rho<-cor( c(E[upper.tri(E)]), c(t(E)[upper.tri(E)]) )*dcor *0.001 
  ...
  {  
    sEH<-svd(EH)
    Theta<-sEH$u[,1:D,drop=FALSE]%*%diag(sqrt(sEH$d[1:D]),nrow=D)*(-.1)
    Alpha<-sEH$v[,1:D,drop=FALSE]%*%diag(sqrt(sEH$d[1:D]),nrow=D)*.3
  }
