...
cv_Auc_leaveoneout<-function(model, K=NULL, D=NULL){
  

    auc = matrix(NA, nrow = 2, ncol = nrow(model$X))
    colnames(auc)=paste("iter",seq(1,nrow(model$X)),sep="")
    rownames(auc)=c("network","responses")
    
    X=model$X
    Y=model$Y
    
    for (n in 1:nrow(model$X)){
      Y.t = leaveoneout_bipartite(Y)[[n]]
      X.t = leaveoneout_network(X)[[n]]
      
      if((sd(c(X.t$test), na.rm = TRUE) !=0)){
        tmp<-jnirt(X=X.t$training, Y=Y,family_network=model$family_network,
