...
def inner_folds_already_done(output_dir: str, outer_fold: int,
                              param_combinations: list, n_inner_folds: int) -> bool:
    """Check if all inner fold results already exist for this outer fold."""
    for fold_idx in range(n_inner_folds):
        for params in param_combinations:
            num_cluster, lr, reassign = params
            run_id = f"outer_fold_{outer_fold}_inner_fold_{fold_idx}_k{num_cluster}_lr{lr}_reassign{reassign}"
            result_file = os.path.join(output_dir, f'inner_fold_results_{run_id}.json')
            if not os.path.exists(result_file):
                logger.info(f"Missing inner fold result: {result_file}")
                return False
    logger.info(f"All inner fold results already exist for outer fold {outer_fold}, skipping.")
    return True


def outer_fold_already_done(output_dir: str, outer_fold: int, params: tuple) -> bool:
    """Check if a valid outer fold result already exists."""
    num_cluster, lr, reassign = params
    run_id = f"outer_fold_{outer_fold}_k{num_cluster}_lr{lr}_reassign{reassign}"
    result_file = os.path.join(output_dir, f'outer_fold_results_{run_id}.json')
    ...
    try:
        with open(result_file, 'r') as f:
            result = json.load(f)
        # Invalid if it has an error key or val_loss is inf
        if 'error' in result:
            logger.info(f"Outer fold result has error, will re-run: {result_file}")
            return False
        if result.get('val_loss', float('inf')) == float('inf'):
            logger.info(f"Outer fold result has inf val_loss, will re-run: {result_file}")
            return False
        logger.info(f"Valid outer fold result exists for {run_id}, skipping.")
        return True
    except Exception as e:
        logger.warning(f"Could not read outer fold result {result_file}: {e}")
        return False
...
def find_best_hyperparams_by_k(output_dir: str, param_combinations: List[tuple],
    ...
    # Collect inner fold results only for current outer fold
    for file in os.listdir(output_dir):
        if file.startswith(f'inner_fold_results_outer_fold_{current_outer_fold}_') and file.endswith('.json'):
            try:
    ...
    for k in k_values:
        k_params = {params: m for params, m in mean_metrics.items() if params[0] == k}
    ...
    # Create visualizations (only meaningful when multiple k values exist)
    try:
...
def nested_cross_validation(data_path: str, script_path: str, output_dir: str = '',
    ...
    Supports resume/patch: skips already completed inner and outer folds.

    For each outer fold:
    1. Run inner CV to find best hyperparameters for each k (skipped if all results exist)
    2. Evaluate each k with its best hyperparameters on outer test set (skipped if valid result exists)
    """
    ...
    try:
        for outer_fold, (train_idx, test_idx) in enumerate(outer_cv.split(all_indices)):
            ...
            # ----------------------------------------------------------------
            # INNER FOLDS: skip entirely if all results already exist
            # ----------------------------------------------------------------
            if inner_folds_already_done(output_dir, outer_fold, param_combinations, n_inner_folds):
                logger.info(f"Skipping inner fold execution for outer fold {outer_fold}")
            else:
                inner_fold_tasks = []
            ...
            # ----------------------------------------------------------------
            # Always re-derive best params from existing inner fold files
            # ----------------------------------------------------------------
            best_params_by_k = find_best_hyperparams_by_k(
                output_dir, param_combinations, current_outer_fold=outer_fold
            )
            ...
            # ----------------------------------------------------------------
            # OUTER FOLDS: skip valid ones, load their results, re-run bad ones
            # ----------------------------------------------------------------
            outer_fold_tasks_to_run = []

            for task in outer_fold_tasks:
                if outer_fold_already_done(output_dir, outer_fold, task.params):
                    # Load existing valid result directly into results list
                    num_cluster, lr, reassign = task.params
                    run_id = f"outer_fold_{outer_fold}_k{num_cluster}_lr{lr}_reassign{reassign}"
                    result_file = os.path.join(output_dir, f'outer_fold_results_{run_id}.json')
                    try:
                        with open(result_file, 'r') as f:
                            all_outer_results.append(json.load(f))
                        logger.info(f"Loaded existing result for {run_id}")
                    except Exception as e:
                        logger.warning(f"Could not load existing result for {run_id}: {e}, will re-run")
                        outer_fold_tasks_to_run.append(task)
                else:
                    outer_fold_tasks_to_run.append(task)

            if outer_fold_tasks_to_run:
                logger.info(f"Running {len(outer_fold_tasks_to_run)} outer fold task(s) for fold {outer_fold}")
                with concurrent.futures.ProcessPoolExecutor(max_workers=max(1, max_workers // 2)) as executor:
                    futures = [executor.submit(process_outer_fold_task, task) for task in outer_fold_tasks_to_run]
                ...
                logger.info(f"All outer fold tasks already done for fold {outer_fold}, skipping execution.")

            # Save progress after each outer fold
