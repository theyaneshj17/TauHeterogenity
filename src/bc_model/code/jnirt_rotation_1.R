...
#' 
#' @export jnirt
jnirt_rotation_1<- function(X, Y,family_network,family_responses, K=2,D=1, model = "bilinear",
...
{ 
  ...
  # a,b,Sab
  E<-Z-intercept 
  a<-rowMeans(E,na.rm=TRUE)*rvar ; b<-colMeans(E,na.rm=TRUE)*cvar*.2
  a[is.na(a)]<-0 ; b[is.na(b)]<-0 
  Sab<-cov(cbind(a,b))*tcrossprod(c(rvar,cvar))*.1
  
  
  
  # beta
  beta<-colMeans(H,na.rm=TRUE)*.1
  EH <- H- beta
  ...
  {  
    sE<-svd(E)
    U<-sE$u[,1:K,drop=FALSE]%*%diag(sqrt(sE$d[1:K]),nrow=K)*.2
    V<-sE$v[,1:K,drop=FALSE]%*%diag(sqrt(sE$d[1:K]),nrow=K)*.5
  ...
  {  
    sEH<-svd(EH)
    Theta<-sEH$u[,1:D,drop=FALSE]%*%diag(sqrt(sEH$d[1:D]),nrow=D)*.3
    Alpha<-sEH$v[,1:D,drop=FALSE]%*%diag(sqrt(sEH$d[1:D]),nrow=D)*.5
  }
  ...
  if(is.null(indices)){
    indices<-matrix(sample(1:nrow(X),min(round(nrow(X)/5),5)*2, replace = FALSE), nrow=2)
  ...
  if(is.null(indices_irt)){
    indices_irt<-matrix(c(sample(1:nrow(X),min(round(nrow(X)/5),5), replace = FALSE),
                          sample(1:ncol(Y),min(round(nrow(X)/5),5), replace = FALSE)), nrow=2, byrow = TRUE)
  }
  ...
  { 
    ...
    if (K > 0)
    {
      if(model == "bilinear"){ 
        ...
        U <- t(mhalf(solve(diag(x=diag(cov(U))))) %*% t(U))
      ...
      if(model == "UV")
      {
        ...
        # V <- V -   colMeans(V)
        U <- t(mhalf(solve(diag(x=diag(cov(U))))) %*% t(U))
        V <- t(mhalf(solve(diag(x=diag(cov(V))))) %*% t(V))
      }
    ...
    Theta <- t(mhalf(solve(diag(x=diag(cov(Theta))))) %*% t(Theta))
    ...
    if(s ==burn){ U_target<-U; Theta_target <- Theta}
    ...
    if(s>burn){
      ...
      ###
      tmp <- Procrustes(Theta, Theta_target,
                        translate = FALSE,
                        dilate = FALSE,
                        sumsq = FALSE)
      Theta<-tmp$X.new
  ...
  if(model == "UV") 
  {
    ...
    fit <- list(BETA=BETA,INT = INT, VC=VC,COV=COV, APM=APM,BPM=BPM,U=UPM,
                Alpha=ALPHAPM,Theta=THETAPM,V=VPM,U_1=U_1,V_1=V_1,UVPM=UVPM,ALPHATHETAPM = ALPHATHETAPM, EZ=EZ,EH=EH,
                XPM=XPM,YPM=YPM,  Theta_1=Theta_1, Alpha_1=Alpha_1,
                GOF=GOF,family_network=family_network,
  ...
  if(model == "bilinear") 
  {
    ...
    rownames(Alpha)<-colnames(Y)
    fit <- list(BETA=BETA,INT = INT, VC=VC,COV=COV, APM=APM,BPM=BPM,
                U_1=U_1,U=UPM,Alpha=ALPHAPM,Theta=THETAPM,UVPM=UVPM,ALPHATHETAPM = ALPHATHETAPM, EZ=EZ,EH=EH,
                XPM=XPM,YPM=YPM, Theta_1=Theta_1, Alpha_1=Alpha_1,GOF=GOF,family_network=family_network,
                family_responses=family_responses,X=X,Y=Y, UVC=UVC, TAC=TAC, input=input)
  }
