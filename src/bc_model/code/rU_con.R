...
#' 
#' @export rU
rU_con<-function(Fl,U, Su, s2, offset=offset)
{
  ...
  if(K==1){U=matrix(sapply(1:V, function(x) rU_con_each(i=x, Es,U,ivU, to, K,N), simplify = TRUE))}
  if(K>1){U=t(sapply(1:V, function(x) rU_con_each(i=x, Es,U,ivU, to, K,N), simplify = TRUE))}
