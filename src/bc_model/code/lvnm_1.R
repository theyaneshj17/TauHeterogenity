...
#' 
#' @export lvnm
lvnm_1<-function (X, family_network, K=2, model = "bilinear",
                rvar = FALSE , cvar = FALSE, dcor = FALSE, independent=TRUE,
                seed = 1, nscan = 10000, burn = 500, odens = 25,indices = NULL,
                print = TRUE, gof=TRUE, 
                prior=list())
{ 
  ...
  { 
    Z<-matrix(rnorm(nrow(X)*nrow(X)),nrow(X),nrow(X)) 
  ...
  # a,b,Sab
  E<-Z-intercept 
  a<-rowMeans(E,na.rm=TRUE)*rvar*2 ; b<-colMeans(E,na.rm=TRUE)*cvar*(-1)
  a[is.na(a)]<-0 ; b[is.na(b)]<-0 
  ...
  {  
    sE<-svd(E)
    U<-sE$u[,1:K,drop=FALSE]%*%diag(sqrt(sE$d[1:K]),nrow=K)*3
    V<-sE$v[,1:K,drop=FALSE]%*%diag(sqrt(sE$d[1:K]),nrow=K)*0.1
  ...
  { 
    ...
    if (K > 0)
    {
      ...
      if(model == "UV")
      {
        ...
        #UV<-rUV_fc((Z-(intercept+outer(a,b,"+"))), U, V,Suv=Su,rho, s2)
        UV<-rUV_network(Z,U,V,Suv=Su,rho,s2,offset=(intercept+outer(a,b,"+")))
        U<-UV$U ; V<-UV$V
        # 
        # E<-Z-(intercept+outer(a,b,"+")) 
        # UV<-rUV_sym_fc(E=.5*(E+t(E)), U, V, s2) 
        # U<-UV$U ; V<-UV$V
      }
